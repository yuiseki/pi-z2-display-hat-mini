...
- 10 分無操作: 跳ね回る地図タイル段階 (maplibre 版と同仕様)。`mbgl-render` で広域 (z4) タイルを数枚 (既定 8、全スタイル×リージョンをシャッフルして先頭 N 個) 背景スレッドでプリレンダして画像キャッシュし、中央を正方形クロップしたタイルを跳ね回らせる。壁にぶつかる度にキャッシュ画像を差し替える (描画ゼロ = プチフリーズ無し)。プリレンダ完了後はレンダリング停止。スタイル: osm-bright-ja / maptiler-basic-ja / osm-fiord。リージョン: Paris / New York / Tokyo / Hiroshima。
- 消灯: バッテリー駆動は 30 分、給電中 (PiSugar の `battery_power_plugged`) は 12 時間でディスプレイ消灯 (バックライト 0、描画停止)。
- 復帰: HAT ボタン (MAP) / キー入力 (TERMINAL) のいずれかで通常表示へ即復帰。復帰のトリガとなった入力は「起こすだけ」で消費し、地図移動・モード切替・文字入力には使わない (誤操作防止)。復帰時は地図タイルの背景レンダスレッドも停止しキャッシュを破棄する。
...
- 共有ヘルパ `apps/screensaver.py` の `Screensaver` クラスが状態 (ACTIVE / SAVER / TILE / OFF)・バックライト・ロゴ/タイル描画を司る。HAT/画面の占有は呼び出し側 (子アプリ) に従い、supervisor は関与しない (8 の排他方針)。
- 子アプリは入力検出時に `note_activity()`、ループ毎に `tick()` を呼ぶ。SAVER 中は `tick()` がロゴを 1 フレーム進める (約 14fps、Pillow 直描画)。TILE 中はキャッシュ済み正方形タイルを跳ね回らせる (バウンド時のみ画像差し替え)。タイルのプリレンダ (`mbgl-render`) は背景スレッドで行い、メインループ (復帰判定) はブロックしない。
- MAP は SAVER/TILE/OFF 中は自前のプリレンダを止める (消費電力と LED 点滅の抑制)。
- 給電判定: `pi-power` の `battery_power_plugged` を 10 秒 TTL でキャッシュ取得。給電中は消灯までを `PI_OFF_AFTER_AC` (既定 12 時間)、バッテリー駆動は `PI_OFF_AFTER` (既定 30 分) とする。
- タイマ/タイルは環境変数で上書き可能 (動作確認・調整用): `PI_SAVER_AFTER` / `PI_TILE_AFTER` / `PI_OFF_AFTER` / `PI_OFF_AFTER_AC` / `PI_TILE_ZOOM` / `PI_TILE_CACHE`。
- 手動トリガ: `pi-screensaver` (別名 `pi-saver`) で待たずに発動を確認できる。制御ファイル `/tmp/pi-display/saver` に `saver` / `tile` / `off` / `wake` を書き、`Screensaver.tick()` が読み取って即座に状態遷移する。状態遷移は `[screensaver] -> SAVER/TILE/OFF/ACTIVE` として stdout (journal) に出る。
  - `pi-screensaver` または `pi-screensaver saver` → 直ちに DVD ロゴ
  - `pi-screensaver tile` → 直ちに跳ねる地図タイル段階
...
│   └── screensaver.py        # アイドル時の DVD ロゴ → 跳ねる地図タイル → 消灯 (両アプリ共有)
├── assets/
